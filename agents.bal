...
Returns the current date and time in RFC 3339 format (e.g., "2026-03-22T14:30:00Z").
...
The key distinction: if the answer would be the same at any company, use general knowledge. If it depends on ConnectWave's specific policies, calendar, or current date context, use the tools.
...
# Converts a given `time:Utc` time to a RFC 3339 timestamp (e.g., `2007-12-03T10:15:30.00Z`). ```ballerina string utcString = time:utcToString(time:utcNow()); ```
...
isolated function getCurrentTime() returns string {
    string result = time:utcToString(time:utcNow());
    return result;
}
